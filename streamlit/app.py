...
@st.cache_resource
def train_als_model(_df_clean):
    # Filtrer les utilisateurs et produits avec suffisamment d'interactions
    min_interactions = 5
    user_counts = _df_clean.groupBy("user_id").count().filter(f"count >= {min_interactions}").select("user_id")
    product_counts = _df_clean.groupBy("product_id").count().filter(f"count >= {min_interactions}").select("product_id")
    
    df_filtered = _df_clean.join(user_counts, "user_id").join(product_counts, "product_id")
    
    df_for_model = df_filtered.select("user_id", "product_id", "rating")
    ...
    df_indexed = user_indexer.fit(df_for_model).transform(df_for_model)
    df_indexed = product_indexer.fit(df_indexed).transform(df_indexed)
    df_indexed = df_indexed.withColumn("user_index", col("user_index").cast("integer"))
    df_indexed = df_indexed.withColumn("product_index", col("product_index").cast("integer"))
    ...
    # Calcul de la précision, rappel et F1 (simplifié pour l'exemple)
    # En pratique, il faudrait utiliser un évaluateur de classement
    true_positives = predictions.filter((col("rating") >= 4) & (col("prediction") >= 4)).count()
    false_positives = predictions.filter((col("rating") < 4) & (col("prediction") >= 4)).count()
    false_negatives = predictions.filter((col("rating") >= 4) & (col("prediction") < 4)).count()
                    ...
                    if recs_list:
                        ...
                        edited_recs = st.data_editor(
                            ...
                            column_config={
                                ...
                                "rating_prédit": st.column_config.NumberColumn(
                                    "⭐ Note prédite",
                                    format="%.2f",
                                    min_value=0,
                                    max_value=5
                        ...
                        if edited_recs['Détails'].any():
                            ...
                            with st.expander(f"🔍 Détails complets pour {selected_rec['title']}", expanded=True):
                                ...
                                st.subheader("⭐ Note prédite pour vous")
                                st.metric("", f"{selected_rec['rating_prédit']:.2f}/5")
